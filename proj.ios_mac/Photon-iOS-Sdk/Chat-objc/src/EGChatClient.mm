...
- (instancetype) initClient:(id<EGChatListener>)listener :(NSString*)applicationID :(NSString*)appVersion :(NSString*)username :(nByte)connectionProtocol :(EGAuthenticationValues*)authenticationValues
{
	self = [super init];
	...
	if(authenticationValues)
		...
		mAuthenticationValues = [[EGAuthenticationValues alloc] retain];
...
- (bool) connect:(NSString* const)nameServerAddress
{
	...
	if([mPeer connect:[NSString stringWithFormat:@"%@%@", nameServerAddress, [nameServerAddress containsString:@":"]?@"":[NSString stringWithFormat:@":%d", (mConnectionProtocol==ConnectionProtocol::UDP?NetworkPort::UDP:NetworkPort::TCP)+NAMESERVER_PORT_GAP]]])
	{
...
- (void) onEvent:(EGEventData*)eventData
{
	...
	switch(eventData.Code)
	{
		...
		case EventCode::PRIVATE_MESSAGE:
		{
			...
			if(mUserId && [sender isEqualToString:mUserId])
