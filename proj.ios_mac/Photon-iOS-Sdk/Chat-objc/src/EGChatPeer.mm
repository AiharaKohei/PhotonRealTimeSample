...
- (bool) opAuthenticateOnNameServer:(NSString* const)appID :(NSString* const)appVersion :(NSString* const)region :(NSString* const)userID :(const EGAuthenticationValues* const)authenticationValues;
{
	...
	if(userID)
		[op setObject:userID forKey:[NSValue value:&ParameterCode::USER_ID withObjCType:@encode(nByte)]];
