...
class PhotonListener : NSObject, EGLoadBalancingListener
{
	...
	{
		if (eventCode == DemoEventCode1)
		{
			if (eventContent is NSValue)
			{
				var v = eventContent as! NSValue;
				var count : int64 = 0
				v.getValue(&count)
				var plNr = 55
...
class NetworkLogic
{
	...
	init(demoView : DemoView)
	{
		...
		self.client = EGLoadBalancingClient(client : listenerRef, appId, appVersion, userName, 0, nil, false, true)
	...
	{
		count++
		var d = NSValue(bytes: &count, objCType: (1 as NSNumber).objCType)
		client.opRaiseEvent(true, d, DemoEventCode1, 0, 0, nil, 0, nByte(EGReceiverGroup_ALL))
