...
#	pragma warning(pop)
		...
		bool Client::connect(const JString& serverAddress, nByte serverType)
		{
			...
			if(mPeer.connect(serverAddress + (serverAddress.indexOf(L':') == -1?JString(L":")+((M_CONNECTION_PROTOCOL==ConnectionProtocol::UDP?NetworkPort::UDP:NetworkPort::TCP)+(serverType==ServerType::NAME_SERVER?NAMESERVER_PORT_GAP:0)):JString())))
			{
						...
						else if(mState == PeerStates::ConnectedToGameserver)
						{
							...
							if(mLastJoinType == JoinType::CREATE_ROOM)
								opCreateRoom(mRoomName, mpCurrentlyJoinedRoom->getIsVisible(), mpCurrentlyJoinedRoom->getIsOpen(), mpCurrentlyJoinedRoom->getMaxPlayers(), mpCurrentlyJoinedRoom->getCustomProperties(), mpCurrentlyJoinedRoom->getPropsListedInLobby(), 
									Common::JString(), LobbyType::DEFAULT, mpCurrentlyJoinedRoom->getPlayerTtl(), mpCurrentlyJoinedRoom->getEmptyRoomTtl()); // lobbyName and lobbyType not used on game server
							else if(mLastJoinType == JoinType::JOIN_ROOM)
								opJoinRoom(mRoomName, mLastJoinCreateIfNotExists, mLastJoinPlayerNumber, mLastCacheSliceIndex, mpCurrentlyJoinedRoom->getPlayerTtl(), mpCurrentlyJoinedRoom->getEmptyRoomTtl());
